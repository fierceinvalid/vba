...
'        strPath = Application.DefaultFilePath
   ...
   '----Add Leading Zero-----
    ...
    For Each wsA In wbA.Worksheets
        With wsA
            LastRow = .Cells.Find("*", After:=Cells(Rows.Count, 1), _
                SearchOrder:=xlRows, SearchDirection:=xlPrevious).Row
            LastColumn = .Cells.Find("*", After:=Cells(1, Columns.Count), _
                SearchOrder:=xlColumns, SearchDirection:=xlPrevious).Column
'           .PrintPreview
        End With
        
        With wsA.PageSetup
            .PrintArea = ActiveSheet.Range(Cells(1, 1), Cells(LastRow, LastColumn)).Address
...
Application.ScreenUpdating = True 'prevents screen flashing through sheets during code
